 ...
 */
export function saveChannelConfig(channelType: string, config: ChannelConfigData): void {
...
export function deleteChannelConfig(channelType: string): void {
  ...
  if (currentConfig.channels?.[channelType]) {
    ...
    writeOpenClawConfig(currentConfig);
    logger.info(`Deleted channel config for ${channelType}`);
    ...
    if (currentConfig.plugins?.entries?.[channelType]) {
      ...
      writeOpenClawConfig(currentConfig);
      logger.info(`Deleted plugin channel config for ${channelType}`);
    }
  }
...
export function setChannelEnabled(channelType: string, enabled: boolean): void {
  ...
  if (PLUGIN_CHANNELS.includes(channelType)) {
    ...
    writeOpenClawConfig(currentConfig);
    logger.info(`Set plugin channel ${channelType} enabled: ${enabled}`);
    return;
  }
  ...
  writeOpenClawConfig(currentConfig);
  logger.info(`Set channel ${channelType} enabled: ${enabled}`);
}
...
export async function validateChannelConfig(channelType: string): Promise<ValidationResult> {
  ...
  try {
    ...
    const output = execSync(`node openclaw.mjs doctor --json 2>&1`, {
      cwd: openclawPath,
      encoding: 'utf-8',
      timeout: 30000,
    });
    ...
    if (errorMessage.includes('Unrecognized key') || errorMessage.includes('invalid config')) {
      ...
      logger.warn('Doctor command failed:', errorMessage);
