...
function EmployeeBreakdown() {
  ...
  const [employeeCredits, setEmployeeCredits] = useState<
    { id: string; name: string; avatar: string; consumed: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const results: { id: string; name: string; avatar: string; consumed: number }[] = [];
        ...
        {employeeCredits.map((emp) => {
          ...
          return (
            <div key={emp.id} className="flex items-center gap-3">
              <PixelAvatar avatar={emp.avatar} size="sm" showStatusRing={false} />
