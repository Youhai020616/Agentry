...
export const useChannelsStore = create<ChannelsState>((set, get) => ({
  ...
  fetchChannels: async () => {
    ...
    try {
      const result = (await window.electron.ipcRenderer.invoke('gateway:rpc', 'channels.status', {
        probe: true,
      })) as {
        success: boolean;
        result?: {
      ...
      if (result.success && result.result) {
        ...
        for (const channelId of channelOrder) {
          const summary = (data.channels as Record<string, unknown> | undefined)?.[channelId] as
          ...
          const anyConnected = accounts.some(
            (a) => a.connected === true || a.linked === true || hasRecentActivity(a)
          );
          ...
          const summaryError =
            typeof (summary as { error?: string })?.error === 'string'
              ...
              : typeof (summary as { lastError?: string })?.lastError === 'string'
                ...
                : undefined;
          const anyError =
            accounts.some((a) => typeof a.lastError === 'string' && a.lastError) ||
          ...
          channels.push({
            ...
            error:
              (typeof primaryAccount?.lastError === 'string'
                ...
                : undefined) || (typeof summaryError === 'string' ? summaryError : undefined),
          });
        }
  ...
  deleteChannel: async (channelId) => {
    ...
    // Use lastIndexOf because channelType itself may contain hyphens
    const lastDash = channelId.lastIndexOf('-');
    const channelType = lastDash > 0 ? channelId.substring(0, lastDash) : channelId;
    ...
    try {
      await window.electron.ipcRenderer.invoke('gateway:rpc', 'channels.delete', {
  ...
  connectChannel: async (channelId) => {
    ...
    try {
      const result = (await window.electron.ipcRenderer.invoke('gateway:rpc', 'channels.connect', {
        channelId,
  ...
  disconnectChannel: async (channelId) => {
    ...
    try {
      await window.electron.ipcRenderer.invoke('gateway:rpc', 'channels.disconnect', { channelId });
    } catch (error) {
  ...
  requestQrCode: async (channelType) => {
    const result = (await window.electron.ipcRenderer.invoke('gateway:rpc', 'channels.requestQr', {
      type: channelType,
