...
3. `xhs publish ...` — 执行发布
4. 如果失败，**不要立即重试**，先用 `xhs search "你的标题关键词"` 确认是否已发布
5. 向用户报告结果

### 流程 B：搜索 → 互动

1. `xhs search "关键词"` — 搜索目标笔记
2. 从结果中获取 `feed_id` 和 `xsec_token`
3. `xhs detail FEED_ID --token TOKEN` — 查看详情（可选）
4. `xhs like/comment/favorite ...` — 执行互动
5. 向用户报告完成情况

### 流程 C：数据分析

1. `xhs analytics` — 拉取创作者看板数据
2. 分析数据趋势
3. 向用户汇报 insights
...
6. **代理** — 如果在海外，MCP 服务需要通过代理访问小红书，通过 `xhs config set mcp.proxy http://127.0.0.1:7897` 配置。
